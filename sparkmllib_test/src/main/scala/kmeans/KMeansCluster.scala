...
object KMeansCluster extends App{
  ...
  //加载数据
  val inPath = "f://MyProject/spark_test/in_data/kmeans_data.txt"
  ...
  // 保存模型
  val outPath = "f://MyProject/spark_test/out_data/KMeansModel"
  clusters.save(sc, outPath)
  val sameModel = KMeansModel.load(sc, outPath)
