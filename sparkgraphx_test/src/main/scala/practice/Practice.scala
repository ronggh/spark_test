...
object Practice extends App{
  ...
  graph.mapTriplets(tri=>tri.srcAttr._2 * tri.dstAttr._2).triplets.collect
    .foreach(e => println(s"${e.srcId} to ${e.dstId} att ${e.attr}"))
  println
  ...
  graph.aggregateMessages[Array[(VertexId,(String,Int))]](ctx =>
    ctx.sendToDst(Array((ctx.srcId.toLong,(ctx.srcAttr._1,ctx.srcAttr._2) ))),_ ++ _)
    .collect.foreach(v => {
    println(s"id: ${v._1}")
    for(arr<- v._2){
      println(s"    ${arr._1} (name: ${arr._2._1}  age: ${arr._2._2})")
